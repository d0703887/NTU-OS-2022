...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
		...
		if(j < NumPhysPages){
			...
			kernel->machine->FreePages--;
			kernel->machine->frameTable[j] = &pageTable[vpn];
			...
			// 把東西存到 disk 裡面
			bcopy(&mainMemory[victim * PageSize], buffer1, PageSize); // copy victim content to buffer 1
			kernel->virtualMemoryDisk->ReadSector(pageTable[vpn].virtualPage, buffer2); // swap vpn content into buffer 2
			bcopy(buffer2, &mainMemory[victim*PageSize], PageSize);  // copy vpn content into main memory 
			kernel->virtualMemoryDisk->WriteSector(pageTable[vpn].virtualPage, buffer1); // swap victim content into disk


			kernel->machine->frameTable[victim]->virtualPage = pageTable[vpn].virtualPage;
			kernel->machine->frameTable[victim]->valid = false;
	
			// 把東西 load 到 main memory 中
			pageTable[vpn].valid = true;
			pageTable[vpn].physicalPage = victim;
			kernel->machine->frameTable[victim] = &pageTable[vpn];
