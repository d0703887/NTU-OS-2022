...
AddrSpace::AddrSpace()
{
    AddrSpace::ID = kernel->machine->ID_num; 
    kernel->machine->ID_num += 1;
}
...
//	cout << "number of pages of " << fileName<< " is "<<numPages<<endl;
    ...
    printf("Thread %d requires %d frame, %d free frame remain\n", AddrSpace::ID, numPages, kernel->machine->FreePages);
...
// then, copy in the code and data segments into memory
	if (noffH.code.size > 0) {
        ...
        for(unsigned int j = 0, i = 0; i < numPages; i++){
            j = 0;
            while(j < NumPhysPages && kernel->machine->usedPhyPage[j] == true)j++;
            if(j < NumPhysPages) {   
                bzero(&kernel->machine->mainMemory[j * PageSize], PageSize);
				kernel->machine->usedPhyPage[j] = true;
                kernel->machine->FreePages--;
				kernel->machine->frameTable[j] = &pageTable[i];
				pageTable[i].physicalPage = j;
				pageTable[i].valid = true;
				pageTable[i].use = false;
				pageTable[i].dirty = false;
				pageTable[i].readOnly = false;
                // Add physical address to FIFO queue
                kernel->machine->fifo.push(j);
